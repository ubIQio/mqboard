...
class Microdot(BaseMicrodot):
    def run(self, host='0.0.0.0', port=5000, debug=False):
        ...
        # asyncio.get_event_loop().create_task(asyncio.start_server(serve, host, port))
...
# dispatcher and to the module config dict in board_config.py

def start(mqtt, config):
    log.info("Web server starting on %s, port %d",config["host"],config["port"])
